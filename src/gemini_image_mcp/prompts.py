"""Prompt builders for Gemini image generation and transformation."""


def build_transformation_prompt(prompt: str) -> str:
    """Build a prompt for image transformation.

    Args:
        prompt: The user's edit instructions.

    Returns:
        Formatted prompt for Gemini image transformation.
    """
...
def build_generation_prompt(prompt: str) -> str:
    ...
    """
    return f"""You are an expert image generation AI assistant specialized in creating visuals based on user \
requests. Your primary goal is to generate the most appropriate image without asking clarifying questions, even \
when faced with abstract or ambiguous prompts.
...
2. **Text Handling Protocol**
   ...
   - For concepts typically associated with text (like "newspaper" or "letter"), create visual \
representations without any legible writing
...
Remember: Your success is measured by your ability to produce satisfying images without requiring \
additional input from users AND without including ANY text from queries in the images. Be decisive and confident \
in your interpretations while maintaining absolute adherence to the no-text requirement.
...
def build_translation_prompt(prompt: str) -> str:
    """Build a prompt for translating text to English.

    Args:
        prompt: The text to translate.

    Returns:
        Formatted prompt for Gemini translation.
    """
    return f"""Translate the following prompt into English if it's not already in English. Your task is ONLY to \
